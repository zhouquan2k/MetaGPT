...
async def test_modify_design():
    design = company.environment.artifact_mgr.create_artifact(ArtifactType.DESIGN, f'{module_name}.md', path="docs",
                                                              parse_mapping=Design_Output_Mapping)
    task = Task(artifact=design, description='''
    - For selecting participants, we use get all users endpoint from 'User' module to get the users in the dropdown select 
    ''')
