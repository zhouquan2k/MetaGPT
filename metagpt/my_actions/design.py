...
ACTION_PREFIX = '''
You are an architect; the goal is to design a java web application
'''
...
- the '...' in examples means content need you to fill in. try to supplement them.
...
## Implementation approach: Provide as Plain text. Analyze the difficult points of the requirements. each line by one point. don't repeat the design from 'System Design' here, just follow it.
...
## UI Design: detail ui design according to the 'UI Design Draft' in prd, should includes: 
    ...
    - 3. endpoints referenced but in implemented in other modules which can be understand by call function 'get_api' to know the detail endpoints.
...
- dependencies: a list of file names that this file has dependency on, like: ServiceImpl depends on Service and DataObject, Vue depends on api.js
...
## Endpoints to implement: use openai 3.0.0 specification to list all the endpoints we should implement in this module. including all the data objects declared in this module.

## Data structures and interface definitions: Use mermaid classDiagram code syntax, including data objects and service interfaces (with type annotations), CLEARLY MARK the RELATIONSHIPS between classes. The data structures SHOULD BE VERY DETAILED and the API should be comprehensive with a complete design. 
NOTICE: all the names must be consistent with the endpoints defined in OpenAPI above.
